...
const getStorageKey = (employeeId: string) => {
  ...
  return `chatbot_state_${userId}_${employeeId}`;
};
const getUserId = () => {
...
export default function VoucherChatbotMVP() {
  ...
  useEffect(() => {
    const init = async () => {
      // 🔥 1. LẤY USER CUỐI
      const lastUser = localStorage.getItem(`last_employee_id_${getUserId()}`);

      if (lastUser) {
        const saved = localStorage.getItem(getStorageKey(lastUser));

        if (saved) {
          const parsed = JSON.parse(saved);
          // 🔥 FIX DONE
          if (parsed.flowState === "done") {
            localStorage.removeItem(getStorageKey(lastUser));
            localStorage.removeItem(`last_employee_id_${getUserId()}`);
          }
          ...
          if (RESUME_ALLOWED.includes(parsed.flowState)) {
            setEmployeeId(lastUser); // 🔥 CỰC QUAN TRỌNG

            setChat(parsed.chat || []);
            setFlowState(parsed.flowState);
            setCustomerName(parsed.customerName || "");
            setCustomerPhone(parsed.customerPhone || "");
            setSelectedBranch(parsed.selectedBranch || null);
            setVoucherImage(parsed.voucherImage || "");
            setSavedLife4CutLink(parsed.savedLife4CutLink || "");
            setSavedVoucher(parsed.savedVoucher || "");
  ...
  useEffect(() => {
    ...
    if (employeeId) {
      ...
      // 🔥 THÊM DÒNG NÀY
      localStorage.setItem(`last_employee_id_${getUserId()}`, employeeId);
    }
  }, [
    chat,
    flowState,
  ...
  const handleCustomerSend = async () => {
    ...
    if (flowState === "awaiting_email") {
      ...
      localStorage.removeItem("last_employee_id");
      return;
    }
...
📍Danh sách các chi nhánh:\n\n` +
  ...
  return (
    <div className="h-dvh bg-slate-100 p-2 md:p-3">
      <div
        className="
mx-auto h-full max-w-[420px] w-full
rounded-[32px]
border border-white/40
bg-[#F3DDE5]
shadow-[0_20px_60px_rgba(0,0,0,0.25),0_5px_15px_rgba(0,0,0,0.15)]
overflow-hidden
"
      >
...
rounded-b-2xl
border border-white/40
shadow-[0_8px_25px_rgba(0,0,0,0.15),0_2px_6px_rgba(0,0,0,0.08)]
backdrop-blur-sm
gap-[20px]
"
            >
                      ...
                      className={`inline-block max-w-[60%] rounded-2xl text-[12px] leading-[1.8] ${
                        msg.role === "bot"
                          ...
                          : msg.role === "system"
                            ...
                            : "bg-[#cfe1f7] px-3 py-2 rounded-2xl rounded-br-md shadow-none"
                      }`}
                    >
                ...
                hover:scale-105"
                          ...
                          return (
                            <>
                              ...
                              {/* ✅ CHECK ICON USER */}
                              {msg.role === "customer" && (
                                <div className="text-[12px] text-gray-500 text mt-0"></div>
                              )}
                            </>
                          );
                        })()}
...
rounded-t-3xl
border border-white/40
shadow-[0_-8px_25px_rgba(0,0,0,0.2),0_-2px_6px_rgba(0,0,0,0.1)]
backdrop-blur-md
"
            >
...
"
                  >
                    <input
                      ...
                      className="w-full bg-transparent outline-none text-[11px] leading-[1] py-[2px] disabled:text-slate-400"
                    />
                  </div>
